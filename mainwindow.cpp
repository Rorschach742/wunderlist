...
{  int res;
    ...
    QMessageBox::information(this,"Take a look","I won't add the event until the name field is empty");
        ...
        else{   ui->tableWidget->setItem(row, column, new QTableWidgetItem(date_catch.toString()+" \n"+name));
...
void MainWindow::on_pushButton_clicked()
    {
        ...
        if(new_name=="hello"){
            ...
            if(chec_starred_state){           
                ...
                std::pair<std::multimap<int, Event>::iterator,
                            std::multimap<int, Event>::iterator> r = cat.archivio.equal_range(0);
                  for (auto it = r.first; it != r.second; ++it) {
                      a=it->second.getPosX();
                    qDebug()<< it->first << " : "; it->second.details();
                  }
                }
